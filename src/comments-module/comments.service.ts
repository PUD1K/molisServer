...
class CommentsService {
  async create(req: Request, res: Response, next: NextFunction) {
    const { body, post_id, parent_comment_id }: { body: string; post_id: number; parent_comment_id?: number } =
      req.body;
  ...
  getAll = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const root_comments_json: JsonCommentType[] = await Promise.all(
      root_comments.map(async (root_comment) => {
  ...
  async getAllChildComments(parent_comment: Comments): Promise<JsonCommentType[]> {
    ...
    const child_comments_json = await Promise.all(
      child_comments.map(async (child_comment) => {
